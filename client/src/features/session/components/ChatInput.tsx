    ...
    <div className="w-full flex justify-center pb-2">
      <form className="flex items-end gap-2 w-full max-w-[600px]" onSubmit={e => { e.preventDefault(); onSend() }}>
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 p-3.5 rounded-lg text-base leading-relaxed bg-warm-50 text-warm-800 resize-none max-h-[200px] overflow-y-auto box-border outline-none focus:outline-none focus:ring-2 focus:ring-earth-500 ring-1 ring-inset ring-warm-200"
        />
        <button type="submit" disabled={loading || !input.trim()} className={`h-11 w-11 rounded-full bg-earth-500 text-warm-50 text-lg border-none shadow-none hover:bg-earth-600 transition-colors disabled:opacity-50 flex items-center justify-center ${loading || !input.trim() ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
