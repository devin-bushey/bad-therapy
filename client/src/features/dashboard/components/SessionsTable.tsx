...
export function SessionsTable({ sessions, loading }: { sessions: TherapySession[]; loading: boolean }) {
  ...
  if (loading) return <div className="p-2.5 text-warm-600 h-80 flex items-center justify-center">Loading sessions…</div>
  return (
        ...
        <table className="w-full border-collapse">
          <tbody>
            {table.getRowModel().rows.length === 0 && (
              <tr><td className="p-2 text-warm-600">No sessions found</td></tr>
            )}
