...
async def create_new_session(request: Request, session: SessionCreate, user=Depends(require_auth)) -> Session:
...
@limiter.limit("60/minute")
async def get_session(request: Request, session_id: str, user=Depends(require_auth)):
    supabase = get_supabase_client()
    result = supabase.table("sessions")\
        .select("*")\
        .eq("id", session_id)\
...
@limiter.limit("60/minute")
async def rename_session(request: Request, session_id: str, session: SessionCreate, user=Depends(require_auth)) -> Session:
    updated_session = update_session(session_id=session_id, name=session.name)
    return Session(**updated_session)

@router.delete("/sessions/{session_id}", status_code=204)
@limiter.limit("60/minute")
async def delete_session(request: Request, session_id: str, user=Depends(require_auth)):
    supabase = get_supabase_client()
